...
Filenames use **Title Case with spaces** matching the page title exactly (e.g. `Andrej Karpathy.md`, `LLM Wiki Pattern.md`). This is the Obsidian-native convention — it ensures `[[Page Title]]` wikilinks resolve without aliases or pipe syntax. Also, trim to reasonable length.
...
When creating `[[wikilinks]]`, use the exact page title (Title Case):
- Correct: `[[Entity Name]]`
- Wrong: `[[entity-name]]` or `[[entity-name|Entity Name]]`
